  ...
  switch (command) {
    ...
    case breadthFirstSearch: {
      ...
      if (!tryParseIntFromInput(&vertexId)) {
        std::cout
            << "invalid input. Input should be of integer type. Try again."
            << std::endl;
        return;
      }
      if (vertexId >= graph.getMatrixSize() || vertexId < 1) {
        std::cout << "out of boudns of expected vetex ids. Try again."
                  << std::endl;
        return;
      }

      std::__1::vector<vertex_id> result =
          graphAlgos.breadthFirstSearch(graph, vertexId);
      std::cout << "result: ";
    ...
    case depthFirstSearch: {
      ...
      std::__1::vector<vertex_id> result =
          graphAlgos.depthFirstSearch(graph, vertexId);
      std::cout << "result: ";
      for (vertex_id id : result) {
        std::cout << id << " ";
      }
    ...
    case shortestBetweenTwo: {
      ...
      distance result = graphAlgos.getShortestPathBetweenVertices(
          graph, firstVertexId, secondVetexId);
...
void ConsoleApp::start() {
  ...
  Graph graph = Graph(matrixReader, vertexMapBuilder, dotBuilder);
